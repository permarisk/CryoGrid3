...
%tsvd - extension for allowing parallel processing to calculate lateral heat exchange
...
%tsvd start parallel call
spmd 
    %mpiInit();
    %---------------define input parameters------------------------------------
    index=labindex();%number identifying the process; change this to e.g. 1 for single realization (non-parallel) run
     %set this index to the number of the worker that fails in order to troubleshoot
    run_number=['test_latflux_',num2str(index)];
...
%tsvd % MPI
PARA.technical.syncTimeStep=1/24;    % time step for synronizing parallel runs
...
%tsvd  new T initial profiles for lake and non-lake 
...
%iSaveSettings([run_number '/' run_number '_settings.mat'], FORCING, PARA, LUT, GRID)  %new functions iSaveXX, since normal save does not work
iSaveSettings([run_number '/' run_number '_settings.mat'], FORCING, PARA, GRID)  %new functions iSaveXX, since normal save does not work
